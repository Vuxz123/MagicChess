...
namespace com.ethnicthv.Other.Network.Server
{
    public class NetworkServer : Common, NetworkSender, NetworkTicker
    {
        // events to hook into
        ...
        protected ReceiveStack receivePipe;
        ...
        private void Listen(int port)
        {
            ...
            try
            {
                ...
                // On linux setting the receive timeout will cause the blocking 
                ...
                listener.Start();
                Debug.Log("[Telepathy] Server: listening port=" + port);
                ...
                while (true)
                {
                    ...
                    var sendThread = new Thread(() =>
                    {
                        ...
                        try
                        {
                            ...
                            NetworkFunction.SendLoop(connectionId, client, connection.sendPipe, connection.sendPending);
                        }
                        ...
                        {
                            Debug.LogError("[Telepathy] Server send thread exception: " + exception);
                        }
                    })
                    {
                        IsBackground = true
                    ...
                    var receiveThread = new Thread(() =>
                    {
                        ...
                        try
                        {
                            ...
                            NetworkFunction.ReceiveLoop(connectionId, client, MaxMessageSize, receivePipe, ReceiveQueueLimit);
                        ...
                        {
                            Debug.LogError("[Telepathy] Server client thread exception: " + exception);
                        }
                    })
                    {
                        IsBackground = true
            ...
            catch (ThreadAbortException exception)
            {
                ...
                Debug.Log("[Telepathy] Server thread aborted. That's okay. " + exception);
            }
            catch (SocketException exception)
            {
                // calling StopServer will interrupt this thread with a
                // 'SocketException: interrupted'. that's okay.
                Debug.Log("[Telepathy] Server Thread stopped. That's okay. " + exception);
            }
            catch (Exception exception)
            {
                // something went wrong. probably important.
                Debug.LogError("[Telepathy] Server Exception: " + exception);
            }
        }
        ...
        public bool Start(int port)
        {
            ...
            Debug.Log("[Telepathy] Server: Start port=" + port);
        ...
        public void Stop()
        {
            ...
            Debug.Log("[Telepathy] Server: stopping...");
        ...
        public bool Send(int connectionId, ArraySegment<byte> message)
        {
            ...
            {
                // find the connection
                if (!clients.TryGetValue(connectionId, out var connection)) return false;
                ...
                {
                    // log the reason
                    Debug.LogWarning($"[Telepathy] Server.Send: sendPipe for connection {connectionId} reached limit of {SendQueueLimit}. This can happen if we call send faster than the network can process messages. Disconnecting this connection for load balancing.");
            ...
            Debug.LogError("[Telepathy] Server.Send: message too big: " + message.Count + ". Limit: " + MaxMessageSize);
            return false;
        }
        ...
        public bool Send(ArraySegment<byte> message)
        {
            ...
            {
                success = Send(i, message) && success;
            }
            return success;
        }
        ...
        {
            // find the connection
            return clients.TryGetValue(connectionId, out var connection) ? ((IPEndPoint)connection.client.Client.RemoteEndPoint).Address.ToString() : "";
        }
        ...
        public bool Disconnect(int connectionId)
        {
            ...
            // just close it. send thread will take care of the rest.
            connection.client.Close();
            Debug.Log("[Telepathy] Server.Disconnect connectionId:" + connectionId);
            return true;
        }
